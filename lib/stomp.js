    ...
    if (this instanceof Frame) {
        ...
        if (typeof data === 'object') {
            ...
            for (var i = 1; i < cmd_hdr_lines.length; i++) {
                var key = cmd_hdr_lines[i].split(':');
                header[key.shift()] = key.join(':');
            }
...
module.exports.ServerFrame = {
    ...
    MESSAGE: function (destination, headers, message) { headers.destination = destination; return Frame( { command: 'MESSAGE', header: headers, body: message } ); },
